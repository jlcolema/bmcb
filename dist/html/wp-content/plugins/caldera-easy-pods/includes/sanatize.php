 ...
 * @author  David Cramer <david@digilab.co.za>
 */
class Settings_Caldera_Easy_Pods_Sanitize {
	...
	public static function apply_sanitization_and_validation( $setting, $value, $config ) {
		...
		$stored = Caldera_Easy_Pods_Options::get_all();
	...
	protected static function apply_sanitization_filter( $setting, $value, $config, $sub_setting = false ) {
		$filter_name = "caldera_easy_pods_{$setting}";
		if (  $sub_setting ) {
			$filter_name = "caldera_easy_pods_{$sub_setting}_{$setting}";
		}
